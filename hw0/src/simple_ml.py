...
def softmax_loss(Z, y):
    ...
    ### BEGIN YOUR CODE
    loss_err=np.mean(np.log(np.sum(np.exp(Z), axis=1))-Z[range(Z.shape[0]), y])
    return loss_err
    pass
    ### END YOUR CODE
...
def softmax_regression_epoch(X, y, theta, lr = 0.1, batch=100):
    ...
    for i in range(num_classes//batch+1):
        ...
        z=x_exp/x_exp_sum.reshape(j-i*batch,1)
        I = np.zeros_like(z)
        I[np.arange(z.shape[0]), y_b] = 1
        grad=(x.T.dot(z-I))/z.shape[0]
        theta[:, :] -= lr * grad
        if j>=num_classes:
            break
    pass
...
def nn_epoch(X, y, W1, W2, lr = 0.1, batch=100):
    ...
    for i in range(num_classes//batch+1):
        ...
        x_b=X[i*batch:j]
        y_b=y[i*batch:j]
        z_b=np.maximum(x_b.dot(W1), 0)
        z_d=np.exp(np.dot(z_b, W2))
        z_d_sum=np.sum(z_d, axis=1).reshape(j-i*batch,1)
        z=z_d/z_d_sum
        I=np.zeros_like(z)
        I[np.arange(j-i*batch), y_b] = 1
        g2=z-I
        g1=np.int64(z_b>0)*(g2.dot(W2.T))
        dw1=x_b.T.dot(g1)/z.shape[0]
        dw2=z_b.T.dot(g2)/z.shape[0]
        W1[:, :] -= lr * dw1
        W2[:, :] -= lr * dw2
        if j>=num_classes:
            break
